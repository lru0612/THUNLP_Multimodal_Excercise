...
class MLLMModel(MLLMPreTrainedModel):
    ...
    def get_vllm_embedding(self, data):
        ...
        if hasattr(self.llm.config, "scale_emb"):
            vllm_embedding = (
                self.llm.model.embed_tokens(data["input_ids"])
                * self.llm.config.scale_emb
            )
        else:
    ...
    def _decode(
        self, inputs_embeds, tokenizer, attention_mask, decode_text=False, **kwargs
    ):
    ...
    def _decode_text(self, result_ids, tokenizer):
        ...
        result_ids = [[id for id in seq if id not in terminators] for seq in result_ids]
        result_text = tokenizer.batch_decode(
            result_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True
        )
        ### <===
        return result_text
    ...
    def generate(
        ...
        **kwargs
        ...
        result = self._decode(
            vllm_embeddings, tokenizer, attention_mask, decode_text, **kwargs
        )
        ### <===
