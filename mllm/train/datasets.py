...
class ModelPreferenceDataset(Dataset):
    def __init__(
        ...
        self.data_path = op.join(
            data_dir, filename.replace(".json", "") + f".{reference_name}_logp.pkl"
        )
    ...
    def __getitem__(self, index):
        ...
        if type(logps) == type([]):
            (
                data_dict["ref_win_logp"],
                data_dict["ref_win_avg_logp"],
                data_dict["ref_win_per_token_logp"],
                data_dict["ref_rej_logp"],
                data_dict["ref_rej_avg_logp"],
                data_dict["ref_rej_per_token_logp"],
            ) = logps
        else:
            (
                data_dict["ref_win_logp"],
