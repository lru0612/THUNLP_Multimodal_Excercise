...
def load_generated_captions(cap_file, image_id_key: str, caption_key: str):
    ...
    # list of int
    if image_id_key in caps[0]:
        if type(caps[0][image_id_key]) == type(0):
            imids = [obj[image_id_key] for obj in caps]
        else:
            imids = [
                int(obj[image_id_key].replace("COCO_val2014_", "").replace(".jpg", ""))
                for obj in caps
            ]
    else:
        if type(caps[0]["metainfos"][image_id_key]) == type(0):
            imids = [obj["metainfos"][image_id_key] for obj in caps]
        else:
            imids = [
                int(
                    obj["metainfos"][image_id_key]
...
"""
By Zihao Yue. Pre-process raw answer jsonl to json list with image_id.
"""


def process_raw_answer_file(question_file, answer_file):
    questions = [json.loads(q) for q in open(question_file, "r")]
    qid2img = {q["question_id"]: q["image"] for q in questions}
    answers = [json.loads(a) for a in open(answer_file, "r")]
    for a in answers:
        a["image_id"] = qid2img[a["question_id"]]
